    ...
    function updateInstallHint(){
      ...
      if(isIOSDevice()){
        ...
        hint.textContent = deferredPrompt ? 'Kliknij „Zainstaluj”, aby dodać ikonę aplikacji.' : 'Kliknij „Zainstaluj”. Jeśli okno się nie pojawi, użyj menu Chrome ⋮ → „Dodaj do ekranu głównego”.';
      }else{
    ...
    function showInstallHelp(){
      ...
      const steps = isIOSDevice()
        ...
        : isAndroidDevice()
          ? ['Otwórz aplikację w Chrome.', 'Kliknij menu ⋮ w prawym górnym rogu.', 'Wybierz „Zainstaluj aplikację” albo „Dodaj do ekranu głównego”.', 'Potwierdź dodanie ikony.']
      ...
      modalBody.innerHTML = `<div class="task-card"><strong>Instrukcja instalacji</strong><ol class="note" style="margin-left:18px">${steps.map(step=>`<li>${escapeHTML(step)}</li>`).join('')}</ol><p class="note">Po instalacji aplikacja będzie otwierać się jak zwykła aplikacja z ekranu telefonu.</p></div>`;
...
<\/script>
    ...
    if(installLaterBtn){ installLaterBtn.addEventListener('click',()=>{ sessionStorage.setItem(installPromptSessionKey,'1'); const banner = document.getElementById('installBanner'); if(banner) banner.classList.remove('show'); }); }
    window.addEventListener('appinstalled',()=>{ deferredPrompt=null; const banner = document.getElementById('installBanner'); if(banner) banner.classList.remove('show'); toast('Aplikacja zainstalowana'); });
    window.addEventListener('load',()=>setTimeout(()=>showInstallBanner(), 650));
